(() => {
  ...
  // Safety: if any critical element missing, stop loudly.
  const required = [
    ["date", dateEl],
    ...
    ["vehicleSize", sizeEl],
    ["package", packageEl],
    ["addonsList", addonsListEl],
    ["sumPackage", sumPackageEl],
    ["sumAddons", sumAddonsEl],
    ["sumFees", sumFeesEl],
    ["sumTax", sumTaxEl],
    ["sumTotal", sumTotalEl],
  ];
  ...
  if (missing.length) {
    console.error("booking.js missing DOM ids:", missing.join(", "));
    return;
  }

  // ===== Business rules =====
  // Customer does NOT choose duration. We only use it for availability + soft hold.
  // Max duration (per your earlier note): 6 hours max.
  function getHoldDurationForDate(dateStr) {
    ...
    // Weekday: 3h hold (180)
    // Weekend: 6h hold (360)
    return isWeekend ? 360 : 180;
  }

  // Pricing
  const HST_RATE = 0.13;
  const FIXED_FEES = 0.0;
  ...
  // Packages (must match Packages page names)
  const PACKAGES = {
    refresh: { label: "Select Series", prices: { small: 89.99, medium: 109.99, large: 124.99 } },
    signature: { label: "Signature Series", prices: { small: 134.99, medium: 164.99, large: 182.99 } },
    showroom: { label: "Showroom Series", prices: { small: 249.99, medium: 299.99, large: 339.99 } },
  };
  ...
  const renderPackageOptions = () => {
    ...
    if (current && PACKAGES[current]) packageEl.value = current;
  };
  ...
  const updateSummary = () => {
    ...
    return { pkgKey, sizeKey, pkgPrice, addons, addonsTotal, fees, tax, total };
  };
  ...
  const populateStartTimes = (slots, noteText) => {
    ...
    for (const s of slots) {
      startTimeEl.appendChild(new Option(timeToLabel(s.start), s.start));
    }
  ...
  const loadAvailability = async () => {
    ...
    try {
      ...
      const url = `${API_BASE}/availability?date=${encodeURIComponent(date)}&duration=${encodeURIComponent(
        durationMin
      )}`;
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      submitBtn.textContent = oldBtnText || "Request Booking";
    }
  };
