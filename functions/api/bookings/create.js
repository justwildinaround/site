...
export async function onRequestPost({ request, env }) {
  ...
  const estimateLine = total > 0 ? `Estimate: $${total.toFixed(2)} ${currency} (package $${packagePrice.toFixed(2)} + add-ons $${addonsTotal.toFixed(2)} + fees $${fees.toFixed(2)} + HST $${tax.toFixed(2)})` : "";
...
if (endMin > closeMin) {
  ...
  const lines = [
    ...
    `<strong>Requested start time:</strong> ${startTime} (duration discussed after we review the job)`,
    ...
    (total > 0 ? `<strong>Estimate:</strong> $${total.toFixed(2)} ${currency} (package $${packagePrice.toFixed(2)} + add-ons $${addonsTotal.toFixed(2)} + fees $${fees.toFixed(2)} + HST $${tax.toFixed(2)})` : ""),
