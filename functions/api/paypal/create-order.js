...
export async function onRequestPost({ request, env }) {
  ...
  const returnUrl = `${baseUrl}/pay?bookingId=${encodeURIComponent(String(bookingId))}&token=${encodeURIComponent(token)}&status=paypal_success`;
  const cancelUrl = `${baseUrl}/pay?bookingId=${encodeURIComponent(String(bookingId))}&token=${encodeURIComponent(token)}&status=paypal_cancel`;
