...
export async function onRequestPost({ request, env }) {
  try {
    ...
    // Send them BACK to the payment portal so the UI can show status
    const successUrl = `${base}/pay?bookingId=${encodeURIComponent(String(row.id))}&token=${encodeURIComponent(token)}&status=success`;
    const cancelUrl  = `${base}/pay?bookingId=${encodeURIComponent(String(row.id))}&token=${encodeURIComponent(token)}&status=cancelled`;
